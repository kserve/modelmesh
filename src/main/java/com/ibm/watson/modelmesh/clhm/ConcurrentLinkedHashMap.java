...
package com.ibm.watson.modelmesh.clhm;
...
/*
 * NOTE: This is a modified version of the ConcurrentLinkedHashMap class
 * from https://github.com/ben-manes/concurrentlinkedhashmap.
 *
 * The modifications introduce absolute ordering by timestamp, so that entries
 * may be inserted based on a specific time.
 */
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V>
  ...
  private ConcurrentLinkedHashMap(Builder<K, V> builder) {
    ...
    // NOTE this is assumed to be the java 8 CHM impl (if not a backport should be used)
...
//  @Override
  ...
  V put(K key, V value, long lastUsed, boolean onlyIfAbsent) {
    ...
    checkNotNull(value);
    if (lastUsed < 0L) {
      throw new IllegalArgumentException("lastUsed must be >= 0");
    }
  ...
  Map<K, Long> orderedLruMap(boolean ascending, int limit) {
	    ...
	    try {
	      ...
	      final int initialCapacity = (weigher == Weighers.entrySingleton())
	          ? Math.min(limit, (int) weightedSize())
	          : 16;
	      final Map<K, Long> map = new LinkedHashMap<>(initialCapacity);
	      final Iterator<Node<K, V>> iterator = ascending
	          ? evictionDeque.iterator()
	          : evictionDeque.descendingIterator();
